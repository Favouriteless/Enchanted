...
public class FamiliarSavedData extends SavedData {
    ...
    private final Map<UUID, FamiliarEntry> entries = new HashMap<>();
     ...
     *
     * @return The {@link FamiliarEntry} associated with the specified {@link Player}, or null if the player does
     * not own a familiar.
     */
    public FamiliarEntry getEntry(UUID uuid) {
    ...
    public void setFamiliar(UUID owner, FamiliarType<?, ?> type, TamableAnimal familiar) {
        entries.put(owner, new FamiliarEntryImpl(type, familiar));
        setDirty();
    }
    ...
    public static class FamiliarEntryImpl implements FamiliarEntry {
        ...
        private FamiliarEntryImpl(FamiliarType<?, ?> type, TamableAnimal familiar) {
            this.uuid = familiar.getUUID();
            this.type = type;
            setNbt(familiar.saveWithoutId(new CompoundTag()));
        }

        private FamiliarEntryImpl(FamiliarType<?, ?> type, UUID uuid, CompoundTag nbt, boolean isDismissed) {
            this.uuid = uuid;
            this.type = type;
