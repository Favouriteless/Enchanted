...
public class PoppetAnimation {
	...
	public void render(MatrixStack matrixStack, float partialTicks, int widthScaled, int heightScaled) {
		...
		float scale = 255.0F * MathHelper.sin((float)Math.pow(2.05F * work - 0.9F, 7) + 0.5F); // Plug this into a graphing tool to see how it scales
		...
		matrixStack.mulPose(Vector3f.YP.rotationDegrees(900.0F * MathHelper.abs(MathHelper.sin(piCurve))));
		matrixStack.mulPose(Vector3f.XP.rotationDegrees(6.0F * MathHelper.cos(work * 8.0F)));
		matrixStack.mulPose(Vector3f.ZP.rotationDegrees(6.0F * MathHelper.cos(work * 8.0F)));
